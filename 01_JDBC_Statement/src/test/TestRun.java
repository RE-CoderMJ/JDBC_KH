...
public class TestRun {
	...
	public static void main(String[] args) {
		 ...
		 * - ResultSet : SELEC문 실행 후 조회된 결과물들이 담겨있는 객체
		 * 
		 * * JDBC 과정 (순서중요)
		 * 1) jdbc driver 등록 : 해당 DBMS(오라클)가 제공하는 클래스 등록
		 * 2) Connection 생성 : 연결하고자 하는 DB정보를 입력해서 해당 DB와 연결하면서 생성
		 * 3) Statement 생성 : Connection 객체를 이용해서 생성  (sql문 실행 및 결과받는객체)
		...
		try {
			// 1) jdbc driver 등록
			Class.forName("oracle.jdbc.driver.OracleDriver"); // ojdbc6.jar 파일을 추가 안했을경우 | 추가는 했는데 오타가 있을경우 => ClassNotFoundException 발생
		...
		*/
		...
		// 2. 내 pc에 db상에 JDBC계정에 TEST테이블에 모든 데이터 조회해보기
		...
		// 실행할 sql문
		String sql = "SELECT * FROM TEST WHERE TNAME LIKE '김%'";
		...
		try {
			...
			// 6)
			while(rset.next()) { // 행 커서 움직여주는 역할, 뿐만아니라 해당 행이 있으면 true 없으면 false
